...
data class Env internal constructor(
    ...
    fun declare(name: String, type: Type, value: Value?, span: Span): Result<Env, InterpreterError> {
        ...
        return Success(copy(bindings = bindings + (name to Binding(type, v))))
    }
    ...
    fun assign(name: String, value: Value, span: Span): Result<Env, InterpreterError> {
        ...
        if (!typeMatches(old.type, value)) return Failure(IncompatibleType(span, old.type, runtimeName(value)))
